...
## import Subject ID
def T1_supervision_load(isGadi, Y_test, ratio=[0.75, 0.25]):
    ...
    Del_Unhealthy_Id2 = Unhealthy['eid']
    ID = np.setdiff1d(ID, Del_Unhealthy_Id2)
    Del_Test_Id3 = Y_test['eid']
    ID = np.setdiff1d(ID, Del_Test_Id3)
    ...
    ypos = np.searchsorted(Full_Id[xsorted], Del_Unhealthy_Id2)
    Unhealthy_Idx = xsorted[ypos]
    ## find the index of the Test ID from the full set "Full_Id"
    # assert np.all(np.intersect1d(Full_Id, Del_Test_Id3) == np.sort(Del_Test_Id3))
    Del_Test_Id3 = np.intersect1d(Full_Id, Del_Test_Id3)
    ypos = np.searchsorted(Full_Id[xsorted], Del_Test_Id3)
    Test_Idx = xsorted[ypos]
    ...
    Y_test_label = Full_table.loc[Test_Idx, baseline_Col]
    ...
    if isGadi:
        Y_train_label, Y_val_label = train_test_split(Y_label, test_size=ratio[1]/(ratio[0]+ratio[1]), shuffle=True)
    else:
        _, Rest = train_test_split(Y_label, test_size=0.05, shuffle=True)
        Y_train_label, Y_val_label = train_test_split(Rest, test_size=1-ratio[0], shuffle=True)
...
def load_T1_NeuroImage(Y, root, doc, followup = False):
    ...
    for item in range(Num):
        if followup:
            ...
            type = '_BL_'
        file_name = 'mwp1' + Y[item].astype('str') + type + doc
