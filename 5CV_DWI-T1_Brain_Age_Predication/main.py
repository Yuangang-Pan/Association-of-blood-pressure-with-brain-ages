...
from network.sfcn3d_DWI import SFCN as SFCN_DWI
from network.sfcn3d_extra_DWI import SFCN as SFCN_extra_DWI
...
class Robust_regression():
    def __init__(self, flag_cuda, learning_rate, weight_decay, train_loader, val_loader, test_loader, followbase_loader, unhealthy_loader, followup_loader, data_type, Extra, Data_Aug, doc, loss, split):
    ...
    def train(self, epoch):
        ...
        for batch_idx, (data, Y) in enumerate(self.train_loader):
            ...
            y_pred, _ = self.SFCN(data, data_extra)
    ...
    def get_final_age_feat(self, data_loader):
        self.SFCN.eval()
        data_id = []
        true_age = []
        pred_age = []
        ...
        for batch_idx, (data, Y) in enumerate(data_loader):
            ...
            y_pred, feat_embed = self.SFCN(data, data_extra)
            data_id.append(ID)
            if self.flag_cuda:
                temp_Age = Age.cpu().data.numpy()
                true_age.append(temp_Age)
                y_pred = y_pred.cpu().data.numpy()
                pred_age.append(y_pred)
                feat_embed = feat_embed.cpu().data.numpy()
                feat.append(feat_embed)
            else:
                temp_Age = Age.data.numpy()
                true_age.append(temp_Age)
                y_pred = y_pred.data.numpy()
                pred_age.append(y_pred)
                feat_embed = feat_embed.data.numpy()
                feat.append(feat_embed)

            del data, data_extra, Age, feat_embed
            torch.cuda.empty_cache()
        ...
        return Final_age, Final_feat_embed

    def process(self, Iter=200):

        doc_type = self.doc + '_CV_' + str(self.split) + '_' + self.Extra + '_' + self.Data_Aug + '_' + str(self.wd) + '_' + self.loss
        model_path = './Results/' + self.type + '/' + doc_type + '.pt'
        for epoch in range(Iter):
            self.train(epoch)
        self.train_age, self.train_feat = self.get_final_age_feat(self.train_loader)
    ...
    def Validation_test(self, epoch=100):
        ...
        self.val_age, self.val_feat = self.get_final_age_feat(self.val_loader)
        self.test_age, self.test_feat = self.get_final_age_feat(self.test_loader)
        self.model_save(model_path, epoch)
    ...
    def Extra_process(self, epoch=100):
        ...
        self.followbase_age, self.followbase_feat = self.get_final_age_feat(self.followbase_loader)
        self.unhealthy_age, self.unhealthy_feat = self.get_final_age_feat(self.unhealthy_loader)
        self.followup_age, self.followup_feat = self.get_final_age_feat(self.followup_loader)
